...
import (
	...
	"github.com/INFINCOIN/blockbook-ICC/bchain"
	"github.com/INFINCOIN/blockbook-ICC/bchain/coins/btc"
	"github.com/INFINCOIN/blockbook-ICC/common"
)

// infincoincashRPC is an interface to JSON-RPC bitcoind service
type infincoincashRPC struct {
	*btc.BitcoinRPC
}
...
// NewinfincoincashRPC returns new infincoincashRPC instance
func NewinfincoincashRPC(config json.RawMessage, pushHandler func(bchain.NotificationType)) (bchain.BlockChain, error) {
...
// Initialize initializes infincoincashRPC instance
func (z *infincoincashRPC) Initialize() error {
	...
	z.Parser = NewinfincoincashParser(params, z.ChainConfig)
...
func (z *infincoincashRPC) GetChainInfo() (*bchain.ChainInfo, error) {
...
func (z *infincoincashRPC) GetBlock(hash string, height uint32) (*bchain.Block, error) {
	var err error
	if hash == "" && height > 0 {
...
func (z *infincoincashRPC) GetTransactionForMempool(txid string) (*bchain.Tx, error) {
	return z.GetTransaction(txid)
}

// GetMempoolEntry returns mempool data for given transaction
func (z *infincoincashRPC) GetMempoolEntry(txid string) (*bchain.MempoolEntry, error) {
